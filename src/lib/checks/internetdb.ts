...
import { classifyPort, serviceFor } from './ports';
import {
  answersOfType,
  dnsQuery,
  makeFinding,
...
// `PortClass`, `classifyPort` and `serviceFor` moved to `./ports` — that file
// carries no server-only dependency, so the dashboard can import the same
// classification the module itself uses. Re-exported here so nothing that
// already imports them from `internetdb.ts` (this module's own tests) breaks.
export { classifyPort, serviceFor, type PortClass } from './ports';

/**
 * The shape of this module's `facts`.
 *
 * `CategoryResult.facts` is typed `Record<string, unknown>` everywhere else —
 * it exists for cross-scan comparison, and every module's shape is different.
 * This one is worth naming because `NetworkExposure.tsx` on the dashboard
 * reads it directly rather than re-deriving it from `findings`, the way
 * `ReportTemplate.tsx` already does for the PDF. `import type` only, so this
 * costs the client bundle nothing — types are erased before anything ships.
 */
export interface InternetDbFacts {
  address: string;
  ports: number[];
  notablePorts: number[];
